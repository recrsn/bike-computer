...
export default function ElapsedTimeField({ segments }: ElapsedTimeFieldProps) {
  const elapsedTimeInSegments = segments
    .filter((segment) => !!segment.end)
    .reduce(
      (acc, segment) =>
        acc + (segment.end?.getTime()! - segment.start.getTime()),
      0
    );
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setTime(
        elapsedTimeInSegments +
          new Date().getTime() -
          lastSegment.start.getTime()
      );
    }, 1000);
    return () => clearInterval(interval);
  }, [running, elapsedTimeInSegments, lastSegment]);

  return <Field title="Time" value={formatElapsedTime(time)} size="large" />;
}
